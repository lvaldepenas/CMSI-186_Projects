...
public class ChangeMaker {
  ...
  */
    public static Tuple makeChangeWithDynamicProgramming(int[] d, int amount) {
      ...
      Tuple t0 = new Tuple(d.length);
      Tuple optimal = new Tuple(d.length);
      //Tuple denominations = Tuple.sort(d);
      Tuple denominations = new Tuple(d);
      int[] array= new int[d.length];

      //System.out.println( Tuple.sort(d) );
      ...
      for( int i = 0; i < denominations.length(); i++ ) {
        ...
        for( int j = 0; j < (amount+1); j++ ) {
          ...
          System.out.println( "          table[" + denominations.getElement(i) + "][" + j + "] ----- " + table[i][j] );
        }
        array[i] = 0;
        optimal = table[i][amount];
        if( i != 0 && table[i][amount].total() < table[i-1][amount].total() ) {
          optimal = table[i][amount];
        }
      }
      return optimal;
